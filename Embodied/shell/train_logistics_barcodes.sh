...
export WANDB_PROJECT="${WANDB_PROJECT:-locateanything}"
export WANDB_RUN_ID="${WANDB_RUN_ID:-logistics-lora}"
...
GPUS=${GPUS:-8}
NNODES=${1:-1}
OUTPUT_DIR=${2:-"work_dirs/logistics_lora"}
NODE_RANK=${NODE_RANK:-0}
PORT=${PORT:-29500}
MASTER_ADDR=${MASTER_ADDR:-"127.0.0.1"}

MODEL_PATH=${MODEL_PATH:-"nvidia/LocateAnything-3B"}
META_PATH=${META_PATH:-"locany_recipe/logistics_recipe.json"}
